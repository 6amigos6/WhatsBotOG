...
async function facebookCommand(sock, chatId, message) {
    try {
        ...
        const isCommand = text && text.trim().startsWith('.fb');
        const url = isCommand ? text.split(' ').slice(1).join(' ').trim() : (text ? text.trim() : '');
        ...
        try {
            const caption = title ? `𝗗𝗢𝗪𝗡𝗟𝗢𝗔𝗗𝗘𝗗 𝗕𝗬 𝗢𝗥𝗨𝗝𝗢𝗩\n\n📝 Title: ${title}` : "𝗗𝗢𝗪𝗡𝗟𝗢𝗔𝗗𝗘𝗗 𝗕𝗬 𝗢𝗥𝗨𝗝𝗢𝗩";
